...
if [ -e "$1" ] && [ -e "$2" ]; then
	java -jar openapi-generator-cli.jar generate -i $1 -c $2 -l nodejs-express-server  -o ../server
else 
    ...
    if [ -e "$openapi" ] && [ -e "$config" ]; then
        java -jar openapi-generator-cli.jar generate -i $openapi -c $config -g nodejs-express-server  -o ../server
